...
void *prepareFFmpeg_(void *args) {
    TVFFmpeg *dnfFmpeg = static_cast<TVFFmpeg *>(args);
    dnfFmpeg->prepareFFmpeg();
    //线程函数一定要return，不然到时候出错半天都找不到错误在哪里
    return 0;

}


TVFFmpeg::TVFFmpeg(JavaCallHelper *javaCallHelper, const char *dataSource)
        : javaCallHelper(javaCallHelper) {
    //这里直接用this->url = const_cast<char *>(dataSource);，如果jni调用env->ReleaseStringUTFChars(dataSource_, dataSource);可能会导致url变成一个悬空指针
...
void TVFFmpeg::prepare() {
    pthread_create(&pid_prepare, NULL, prepareFFmpeg_, this);
}


void TVFFmpeg::prepareFFmpeg() {
    //4.x不用调用av_regiest_all了
...
void *startThread(void *args) {
    TVFFmpeg *ffmpeg = static_cast<TVFFmpeg *>(args);
    ffmpeg->play();
    return 0;
}
...
void TVFFmpeg::setRenderCallback(RenderFrame renderFrame) {
